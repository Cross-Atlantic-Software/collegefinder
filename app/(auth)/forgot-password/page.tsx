          ...
          <div className="relative">
            ...
            <section className="relative z-10 rounded-t-[32px] sm:rounded-[20px] border-t-2 sm:border-2 border-black bg-white p-6 pb-8 sm:p-7 text-start shadow-[0_-8px_30px_rgba(0,0,0,0.12)] sm:shadow-sm">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Enter the email linked to your UniTracko account.
              </p>
              <p className="mt-1 text-sm text-slate-500">
              ...
              {done ? (
                <div className="mt-6 space-y-4">
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                    If an account exists with this email, we&apos;ve sent a reset link. Please check your inbox and spam
                    folder.
                  </div>
                  {resetData?.devSkipReason && resetData.devHint && (
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-800">
                      ...
                      <p className="mt-1 text-slate-700">{resetData.devHint}</p>
                    </div>
                  )}
                  {/* {resetData?.resetLink && (
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-800">
                      <p className="font-semibold">Development / fallback</p>
                      {resetData.devHint && <p className="mt-1 text-slate-700">{resetData.devHint}</p>}
                      {resetData.emailDelivered === false && (
                        <p className="mt-2 text-slate-700">
                          SMTP did not send the message. Open the link below to set a new password, or fix{" "}
                          <code className="rounded bg-slate-200 px-1">EMAIL_HOST</code>, <code className="rounded bg-slate-200 px-1">EMAIL_USER</code>, and{" "}
                          <code className="rounded bg-slate-200 px-1">EMAIL_PASS</code> in the backend{" "}
                          <code className="rounded bg-slate-200 px-1">.env</code> (Gmail needs an app password if 2FA is on).
                        </p>
                      )}
                      <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
                        <Link
                          href={resetData.resetLink}
                          className="inline-flex flex-1 items-center justify-center rounded-full bg-slate-900 px-4 py-2.5 text-center text-sm font-semibold text-white transition hover:bg-slate-800"
                        >
                          Open reset page
                        </Link>
                        <button
                          ...
                          className="inline-flex items-center justify-center rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 hover:bg-slate-100"
                        >
                          Copy link
                        </button>
                      </div>
