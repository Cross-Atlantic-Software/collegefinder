...
import {
  ...
  type ScholarshipExamsCollegesMappingRow,
} from '@/api/admin/scholarship-exams-colleges-mappings';
...
export default function ScholarshipExamsCollegesMappingPage() {
  const router = useRouter();
  const { showSuccess, showError } = useToast();
  const { canDownloadExcel } = useAdminPermissions();
  const [mappings, setMappings] = useState<ScholarshipExamsCollegesMappingRow[]>([]);
  ...
  const load = async () => {
    try {
      setIsLoading(true);
      const res = await getAllScholarshipExamsCollegesMappings();
  ...
  const handleTemplate = async () => {
    ...
    try {
      await downloadScholarshipExamsCollegesMappingTemplate();
  ...
  const handleDownloadMappingExcel = async () => {
    ...
    try {
      await downloadScholarshipExamsCollegesMappingsExcel();
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    setUploadError(null);
    try {
      const res = await bulkUploadScholarshipExamsCollegesMappings(file);
      if (res.success && res.data) {
        setResult({
  ...
  const handleDeleteRow = async (m: ScholarshipExamsCollegesMappingRow) => {
    if (
      !window.confirm(
        `Clear exam and college links for this scholarship?\n\n${m.scholarship_name}\n\nThis cannot be undone.`,
      )
    ) {
      return;
    }
    setDeletingId(m.id);
    try {
      const res = await deleteScholarshipExamsCollegesMapping(m.id);
      if (res.success) {
  ...
  const handleDeleteAll = async () => {
    if (
      !window.confirm(
        'Delete ALL scholarship exam and college links?\n\nEvery row in scholarship_exams and scholarship_colleges will be removed. This cannot be undone.',
      )
    ) {
      return;
    }
    setDeletingAll(true);
    try {
      const res = await deleteAllScholarshipExamsCollegesMappings();
      if (res.success) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <AdminHeader title="Scholarship exams & colleges" />
        <main className="flex-1 p-4 md:p-6">
          <div className="mx-auto max-w-4xl space-y-6">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Upload an Excel file with columns <strong>scholarship_names</strong>, <strong>exams</strong>, and{' '}
              <strong>colleges</strong>. The scholarship name must match a record in your scholarships table;
              exams and colleges are resolved by name. Multi-value columns use commas, semicolons, or new lines.
              Duplicate scholarship names in one file: <strong>last row wins</strong>. Re-uploading replaces
              that scholarship&apos;s linked exams and colleges.
            </p>
            ...
            {uploadError && (
              <div className="mt-3 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-900 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-100">
            ...
            {result && (
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm dark:border-slate-800 dark:bg-slate-900/50">
                ...
                {(result.errorDetails ?? []).length > 0 && (
                  <ul className="mt-2 max-h-48 list-inside list-disc space-y-1 overflow-y-auto text-slate-700 dark:text-slate-300">
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
              ...
              {isLoading ? (
                ...
                <p className="p-4 text-sm text-slate-500">No scholarships yet. Add scholarships first.</p>
              ) : (
                <>
                    ...
                    <table className="w-full min-w-[640px] text-left text-sm">
                      ...
                      <tbody>
                        {paginatedMappings.map((m) => (
                          ...
                          >
                            <td className="px-3 py-2 text-slate-800 dark:text-slate-200">{m.scholarship_name}</td>
                            ...
                            <td className="whitespace-nowrap px-3 py-2 text-right">
                              <button
                                ...
                                title="Clear links for this scholarship"
                              >
                                <FiTrash2 className="h-3.5 w-3.5" />
                                Delete
                              </button>
