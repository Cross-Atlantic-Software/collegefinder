...
export function CollegeDetailRecommendedExams({ college }: CollegeDetailRecommendedExamsProps) {
  const examCards = useMemo(
    () =>
      (college.linkedExams ?? []).map((ex) => ({
        id: ex.id,
        exam: ex.code?.trim() || ex.name,
        track: ex.code?.trim() && ex.name?.trim() && ex.code.trim() !== ex.name.trim()
          ? ex.name
          : "Linked exam",
        reason: "Recommended or required for admission to this college.",
        href: examDetailHref(ex.id, "dashboard-college-shortlist"),
      })),
    [college.linkedExams]
  );
