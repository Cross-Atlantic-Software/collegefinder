...
import { FaArrowRight, FaArrowLeft, FaSearch, FaPlus, FaTimes } from "react-icons/fa";
...
function TabStrip({ tabs, activeId, onSelect, onClose, onNew }: TabStripProps) {
  const scrollRef = useRef<HTMLDivElement>(null);

  // Auto-scroll active tab into view
  useEffect(() => {
    const el = scrollRef.current?.querySelector(`[data-tabid="${activeId}"]`) as HTMLElement | null;
    el?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "nearest" });
  }, [activeId]);

  return (
    <div
      className="relative flex h-10 items-end gap-0 overflow-hidden bg-[#EAEEF3] px-3 pt-0 dark:bg-slate-800"
      style={{ boxShadow: "inset 0 -1px 0 #d1d9e0" }}
    >
      {/* scrollable tab list + inline + button */}
      <div
        ref={scrollRef}
        className="flex min-w-0 flex-1 items-end gap-[2px] overflow-x-auto pb-0 scrollbar-hide"
      >
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={[
                "group relative flex h-9 min-w-[120px] max-w-[200px] flex-shrink-0 items-center gap-2 rounded-t-lg border-x border-t px-3 text-left text-[12px] font-medium transition-all duration-150 select-none",
                isActive
                  ? "z-10 border-[#d1d9e0] bg-white text-slate-800 shadow-[0_1px_0_white] dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
                  : "border-transparent bg-[#D9DFE7] text-slate-500 hover:bg-[#CDD4DC] hover:text-slate-700 dark:bg-slate-700 dark:text-slate-400 dark:hover:bg-slate-600",
              ].join(" ")}
              style={{
                clipPath: isActive
            ...
            >
              ...
              {/* label */}
              <span className="flex-1 truncate leading-none">{tab.label}</span>
                ...
                <span
                  ...
                  className={[
                    ...
                    isActive
                      ...
                      : "text-transparent group-hover:text-slate-400 group-hover:hover:bg-[#BCC4CC]",
                  ].join(" ")}
                >
        ...
        <button
          type="button"
          onClick={onNew}
          aria-label="Open new counselling session"
          className="mb-1 ml-1 flex h-7 w-7 flex-shrink-0 items-center justify-center self-end rounded-full text-slate-400 transition-all hover:bg-slate-200 hover:text-slate-600 active:scale-90 dark:hover:bg-slate-700 dark:hover:text-slate-300"
        >
...
/* ─── Session Panel (single tab's content) ───────────────────────────── */
interface SessionPanelProps {
  tab: TabSession;
  onChange: (patch: Partial<TabSession>) => void;
}

function SessionPanel({ tab, onChange }: SessionPanelProps) {
  const [scribbleOn, setScribbleOn] = useState(false);

  const { step, selectedExamId, rankSource, rankInput, submittedRank, submittedSource } = tab;
  ...
  const handleShowResults = () => {
    ...
    onChange({
      submittedRank: parsedRank,
      submittedSource: rankSource,
      step: "loading",
    });
    setTimeout(() => onChange({ step: "results" }), 1800);
  };
  ...
  if (step === "loading" || step === "results") {
    return (
      <div className="relative flex h-full flex-col overflow-hidden bg-[#F6F8FA] text-slate-900 dark:bg-slate-950 dark:text-slate-50">
        {/* header bar */}
        <header className="sticky top-0 z-30 flex flex-col items-start justify-between gap-4 border-b border-slate-200 bg-white/95 px-4 py-3 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/95 md:px-6 md:py-4 lg:flex-row lg:items-center">
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => onChange({ step: "rank" })} aria-label="Go back"
        ...
        {/* body — add bottom padding when results are shown so cards don't hide under footer */}
        {step === "loading" ? (
          <ShimmerGrid />
        ) : (
          <div
            className="flex-1 overflow-y-auto bg-[#f8fbff] p-4 dark:bg-slate-950/40 md:p-6"
            style={{
              animation: "fade-in 300ms ease-out",
              paddingBottom: step === "results" ? "80px" : undefined,
            }}
          >
            {colleges.length === 0 ? (
        ...
        {/* absolute-fixed bottom bar — always pinned regardless of scroll */}
        {step === "results" && (
          <div className="absolute bottom-0 left-0 right-0 z-30 border-t border-slate-200 bg-white/95 px-4 py-3 shadow-[0_-4px_16px_-4px_rgba(0,0,0,0.08)] backdrop-blur dark:border-slate-800 dark:bg-slate-900/95 md:px-6">
            <div className="mx-auto flex w-full max-w-7xl justify-end">
              <Button variant="themeButton" size="sm" onClick={handleStartOver}
                className="inline-flex items-center gap-2 !rounded-full !border-black !bg-black px-5 !text-white shadow-sm transition-all hover:!bg-black/90 active:scale-95">
  ...
  /* ── WIZARD STEPS (welcome / exam / rank) ── */
  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center overflow-y-auto bg-[#F6F8FA] dark:bg-slate-950 px-4">
      <div className="w-full max-w-[500px] py-6">
        <article className="flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm dark:bg-slate-900">
            ...
            <div className="border-b border-slate-100 px-5 pt-5 pb-4 dark:border-slate-800"
              ...
              <div className="flex items-center gap-3 mb-5">
                <button type="button" aria-label="Go back"
                  onClick={() => onChange({ step: step === "exam" ? "welcome" : "exam" })}
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 transition-all hover:border-slate-300 hover:bg-slate-50 active:scale-90 dark:border-slate-700 dark:bg-slate-800">
                  <FaArrowLeft className="h-3 w-3" />
                </button>
                <DelayedLottie key={`hdr-${step}-${tab.id}`} data={currentLottie} className="h-9 w-9 shrink-0" />
          ...
          <div className="min-h-[280px]">
            <div key={`${tab.id}-${step}`} className="p-5" style={{ animation: "fade-in 260ms cubic-bezier(0.4,0,0.2,1)" }}>

              {/* welcome */}
              {step === "welcome" && (
                <div className="flex flex-col items-center py-6 text-center space-y-6">
                  <DelayedLottie key={`welcome-${tab.id}`} data={folderAnim} className="h-24 w-24" />
                  ...
                  <Button variant="themeButton" size="sm" onClick={() => onChange({ step: "exam" })}
                    ...
                    {DUMMY_COUNSELLING_EXAMS.map((exam) => {
                      ...
                      return (
                        <button key={exam.id} type="button"
                          onClick={() => onChange({ selectedExamId: sel ? null : exam.id })}
                          className={[
              ...
              {step === "rank" && (
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {RANK_OPTIONS.map((opt) => {
                        ...
                        return (
                          <button key={opt.id} type="button" onClick={() => onChange({ rankSource: opt.id })}
                  ...
                  <div>
                    <label htmlFor={`counselling-rank-${tab.id}`} className="mb-1.5 block text-xs font-medium text-slate-700 dark:text-slate-300">
                      {RANK_SOURCE_LABELS[rankSource]}
                    </label>
                    <input id={`counselling-rank-${tab.id}`} type="text" inputMode="numeric" pattern="[0-9]*"
                      value={rankInput} onChange={(e) => onChange({ rankInput: e.target.value.replace(/\D/g, "") })}
          ...
          {(step === "exam" || step === "rank") && (
            <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3 dark:border-slate-800">
              ...
              {step === "exam" && (
                <Button variant="themeButton" size="sm" disabled={!selectedExamId}
                  onClick={() => {
                    const exam = DUMMY_COUNSELLING_EXAMS.find(e => e.id === selectedExamId);
                    onChange({ step: "rank", label: exam ? exam.name : "New Session" });
                  }}
...
export default function Counselling() {
  ...
  const updateTab = useCallback((id: string, patch: Partial<TabSession>) => {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] w-full flex-col bg-[#F6F8FA] dark:bg-slate-950">
      {/* Chrome-style tab strip */}
      <TabStrip
        tabs={tabs}
        activeId={activeId}
      ...
      {/* active session */}
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden" key={activeId}>
        <SessionPanel
          tab={activeTab}
