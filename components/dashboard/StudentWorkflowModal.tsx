...
import { FiX, FiPlay, FiRefreshCw, FiCheck, FiAlertCircle, FiLock } from 'react-icons/fi';
...
export function StudentWorkflowModal({
    ...
    // Auto-start workflow when modal opens
    useEffect(() => {
    ...
    const startWorkflow = () => {
        ...
        wsRef.current = connectToWorkflow(String(examId), String(user.id), handlers);
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="relative w-full max-w-6xl h-[90vh] bg-gray-900 rounded-xl shadow-xl flex flex-col">
                ...
                <div className="flex items-center justify-between p-4 border-b border-gray-700">
                    <div>
                        <h2 className="text-xl font-bold text-white">Exam Registration Automation</h2>
                        <p className="text-sm text-gray-400">
                            {examName} • {user?.name || user?.email || 'Student'}
                        </p>
                    </div>
                ...
                <div className="flex-1 flex overflow-hidden">
                        ...
                        <div className="flex-1 bg-gray-800 rounded-lg overflow-hidden flex items-center justify-center">
                            {screenshot ? (
                                ...
                                <p className="text-gray-500">
                                    {status === 'idle' ? 'Starting automation...' : 'Waiting for screenshot...'}
                                </p>
                            )}
                        </div>
                    ...
                    <div className="w-80 border-l border-gray-700 p-4 flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto bg-gray-800 rounded-lg p-3 font-mono text-xs">
                            {logs.length === 0 ? (
                                <p className="text-gray-500">Starting automation...</p>
                            ) : (
                                logs.map((log, i) => (
                                    <div
                                        key={i}
                ...
                <div className="p-4 border-t border-gray-700 flex justify-between">
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            {status === 'success' || status === 'failed' || status === 'stopped' ? 'Close' : 'Cancel'}
                        </button>
                ...
                {showOtpModal && (
                    <InputModal
                        title="Enter OTP"
                        description="Please enter the OTP sent to your registered mobile/email"
                        value={inputValue}
